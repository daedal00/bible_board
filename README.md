...
│   ├── leaderboard.js        -> Reaction leaderboard functionality
├── test
...
- `/awwww` - Drops cute pictures from r/aww into your channel
- `/invite` - Get an invite link for the bot
- `/leaderboard` - Shows the weekly reactors leaderboard for the configured channel
- Weekly automatic leaderboard posting every Monday at 07:05 UTC
- Admin routes for managing the bot:
  - `/register?key=ADMIN_KEY` - Register the leaderboard command
  - `/cron-test?key=ADMIN_KEY` - Test the leaderboard generation with a 30-minute scan
...
# Required environment variables (configured in wrangler.toml)
SOURCE_CHANNEL_ID="123..."  # Channel to scan for reactions
POST_CHANNEL_ID="123..."    # Channel to post leaderboard
GUILD_ID="123..."          # Server ID for command registration
```
